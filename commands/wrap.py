...
from schema import GameStatus, EventType, PRICE_LIST
from strings import String


@MJGCommands.command('wrap')
def eat(bot, update):
    """
    ...
    if chat.type == Chat.PRIVATE:
        ...
        if game:
            event = DB.create_event(session,
                                    ...
                                    type=EventType.WRAP_TOUCH,
                                    created_by=user.username)

            send_player_select_keyboard(bot=bot,
                                        game_id=game.id,
                                        event_id=event.id,
                                        action=String.ACTION_WRAP_TOUCH_SELECT_WINNER,
                                        chat_id=game.chat_id,
                                        text=String.WRAP_TOUCH_ASK_WINNER,
...
@MJGCommands.callback(String.ACTION_WRAP_TOUCH_SELECT_WINNER)
...
@MJGCommands.callback(String.ACTION_WRAP_TOUCH_SELECT_LOSER)
...
@MJGCommands.callback(String.ACTION_WRAP_TOUCH_SELECT_FAN)
def eat_fan(bot, update):
    ...
    if event and not event.completed and current_user.username == event.created_by:
        ...
        text = random.choice(String.WRAP_TOUCH_MESSAGES).format(winner_first=to_player.first_name,
