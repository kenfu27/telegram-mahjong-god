...
def stats(bot, update):
    ...
    with codecs.open('templates/stats', 'r', 'utf-8') as f:
        html = f.read().format(
            ...
            average_eat_amount="{0:.1f}".format(float(stats.eat_amount) / float(stats.eat)) if stats.eat else '0',
            ...
            average_lose_amount="{0:.1f}".format(float(stats.lose_amount) / float(stats.lose)) if stats.lose else '0',
            ...
            average_touch_win_amount="{0:.1f}".format(
                float(stats.touch_win_amount) / float(stats.touch_win)) if stats.touch_win else '0',
            ...
            average_touch_lose_amount="{0:.1f}".format(
                float(stats.touch_lose_amount) / float(stats.touch_lose)) if stats.touch_lose else '0',
            wrap_touch=str(stats.wrap_touch).rjust(5),
            draw=str(stats.draw).rjust(5),
            draw_rate="{0:.4f}".format(float(stats.draw) / float(stats.total_games))
        )
